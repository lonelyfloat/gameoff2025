...
all:
	gcc $(FILES) -o build/$(PROJECT_NAME) $(CFLAGS) -Iinclude -Isrc -Iframework $(LIBLOC) $(LDFLAGS)

editor-reload-lib: 
	gcc -shared -o build/$(EDITOR_NAME).so $(shell $(EDITOR_LIB_FILT)) -Isrc -Iinclude -Iframework -L$(DYNLIBLOC) $(CFLAGS) $(LDFLAGS) -fPIC

reload-lib: 
	gcc -shared -o build/$(PROJECT_NAME).so $(shell $(GAME_LIB_FILT))  -Isrc -Iinclude -Iframework -L$(DYNLIBLOC) $(CFLAGS) $(LDFLAGS) -fPIC
